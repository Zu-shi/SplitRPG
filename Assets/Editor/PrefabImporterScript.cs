...
public class PrefabImporterScript : MonoBehaviour {
	...
	private void ImportPrefabsFromIamges(string map){
		...
		objname = "FencePost";
		children = new string[]{"TopLeft", "TopRight", "MiddleLeft", "MiddleRight", "BottomLeft", "BottomRight"};
		go  = getOriginalPrefabOfObject (objname);
		foreach(string childName in children){
			GameObject temp = go.transform.FindChild(childName).gameObject;
			Utils.assert(temp != null);
